...
навыки использования Grunt + RequireJS и начал все сначала, теперь уже с 
Gulp и Browserify, что бы понять из за чего, собственно, вокруг них вся эта суматоха.

Что бы спасти вас от необходимости гуглить, копаться в документации, проходить 
...
в [Wiki ЧАВО][3] репозитория некоторые из таких базовых понятий, что бы заполнить
...
перемещаю в папку с результатами сборки, указанную в `gulp.dest`. Для того, что бы
...
просто. Что бы разобраться более глубоко [посмотрите наш репозиторий для начинающих][2] 
...
папку node_modules в корневом директории проекта. Флаг `--save` автоматически
...
Browserify API, что бы дать ему возможность опознавать эти разрешения и нам не 
...
Что бы разобраться как можно применять трансформации к CoffeeScript и Handlebars, 
